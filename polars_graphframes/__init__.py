...
def get_cluster_ids(node_definition: IntoExpr, edge_definitions: Iterable[IntoExpr]) -> pl.Expr:
    ...
    edge_definitions = [edge_definition if isinstance(edge_definition, pl.Expr) else F.col(edge_definition) for edge_definition in edge_definitions]

    return register_plugin_function(
        args=[node_definition.hash(seed=0)] + [edge_definition.hash(1 + i) for i, edge_definition in enumerate(edge_definitions)],
        plugin_path=LIB,
